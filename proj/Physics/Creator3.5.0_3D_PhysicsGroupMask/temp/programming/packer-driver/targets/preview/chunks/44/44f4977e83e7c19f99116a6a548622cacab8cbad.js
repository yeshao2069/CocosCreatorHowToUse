      ...
      _export("GroupMaskTest", GroupMaskTest = (_dec5 = ccclass('GroupMaskTest'), _dec6 = property({
        ...
        start() {
          this.items.forEach(i => {
            if (i.target) {
              var c = i.target.getComponent(Collider);
              c.setGroup(i.group);
              c.setMask(i.mask);
            }
          });
        ...
        setItemMaskToNone(event, index) {
          ...
          var c = this.items[int].target.getComponent(Collider);
          c.setMask(EPHY_MASK.M_NONE);
          this.items.forEach(i => {
            if (i.target) {
              var _c = i.target.getComponent(RigidBody);
...
//# sourceMappingURL=44f4977e83e7c19f99116a6a548622cacab8cbad.js.map