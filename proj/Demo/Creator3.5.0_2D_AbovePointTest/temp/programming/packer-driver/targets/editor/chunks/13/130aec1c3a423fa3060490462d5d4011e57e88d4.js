    ...
    execute: function () {
      ...
      _export("Typescript", Typescript = (_dec = ccclass('Typescript'), _dec(_class = class Typescript extends Component {
        ...
        rotateTest() {
          ...
          let b = new Vec3(rectPos.x - size.width / 2, rectPos.y + size.height / 2, 0);
          let c = new Vec3(rectPos.x - size.width / 2, rectPos.y - size.height / 2, 0);
          let d = new Vec3(rectPos.x + size.width / 2, rectPos.y - size.height / 2, 0); // 旋转前
        ...
        rectangleContainPoint(targetPos, rectSize = new Size(), rectPos = new Vec3()) {
          // 计算边界值
        ...
        rectangleContainPointExt(targetPos, angle, rectSize = new Size(), rectPos = new Vec3()) {
          // 计算边界值
          let leftX = rectPos.x - rectSize.width / 2;
          let rightX = rectPos.x + rectSize.width / 2;
          let topY = rectPos.y + rectSize.height / 2;
          ...
          let tPosX = targetPos.x;
          let tPosY = targetPos.y; // 获取旋转后的坐标
          ...
          let radius = Math.PI / 180 * -angle;
          let getSin = parseFloat(Math.sin(radius).toFixed(3));
          let getCos = parseFloat(Math.cos(radius).toFixed(3)); // console.log(getSin,getCos);

          let xOld = (tPosX - rectPos.x) * getCos - (tPosY - rectPos.y) * getSin + rectPos.x;
          let yOld = (tPosY - rectPos.y) * getCos + (tPosX - rectPos.x) * getSin + rectPos.y; // 思路: 将旋转的矩形转换成旋转前(angle=0°时)的矩形，然后把需要判断的节点也转换成旋转前的节点然后进行判断
        ...
        drawRect(aPos, bPos, cPos, dPos, dColor = Color.BLACK) {
...
//# sourceMappingURL=130aec1c3a423fa3060490462d5d4011e57e88d4.js.map