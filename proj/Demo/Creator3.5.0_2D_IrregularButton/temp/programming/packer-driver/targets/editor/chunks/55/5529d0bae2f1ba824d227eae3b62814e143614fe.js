  ...
  return {
    ...
    execute: function () {
      ...
      _export("default", IrregularButton = (_dec = ccclass('IrregularButton'), _dec2 = property(Node), _dec(_class = (_class2 = class IrregularButton extends Component {
        ...
        onTap(e) {
          let touchPos = e.getUILocation(); // 每个子节点与点击坐标的距离集合

          let dis_list = []; // 遍历所有子节点坐标得出与鼠标的距离然后升序排列

          let dis_list_sort = this.childWposList.map(wpos => {
            let tmpVec2 = new Vec2(wpos.x, wpos.y);
            let dis = tmpVec2.subtract(touchPos).lengthSqr(); // 记录距离值

            dis_list.push(dis);
            return dis;
          }).sort((a, b) => a - b); // 获取排序最前面的索引

          let nearest_index = dis_list.indexOf(dis_list_sort[0]); // 颜色修改
          // 先全部恢复白色
...
//# sourceMappingURL=5529d0bae2f1ba824d227eae3b62814e143614fe.js.map