  ...
  return {
    ...
    execute: function () {
      ...
      _export("BezierData", BezierData = class BezierData {
        ...
        createCurve(pos) {
          var end = this.createPoint((_crd && Ident === void 0 ? (_reportPossibleCrUseOfIdent({
            error: Error()
          }), Ident) : Ident).point, pos);
          var control = this.createPoint((_crd && Ident === void 0 ? (_reportPossibleCrUseOfIdent({
            error: Error()
          }), Ident) : Ident).control, pos); // 把曲线列表最后一个点作为新曲线起点

          var start = this._bezierCurveLists[this._bezierCurveLists.length - 1].end;
          var curve = {
            start,
            control,
            end
        ...
        createThirdOrderCurve(pos) {
          ...
          var control2 = this.createPoint((_crd && Ident === void 0 ? (_reportPossibleCrUseOfIdent({
            error: Error()
          }), Ident) : Ident).control, pos); //计算偏移点

          var c1pos = new Vec3(200, 200, 0).add(start.position);
          c1pos.x = Math.min(this._resolution.width / 2, c1pos.x);
          c1pos.y = Math.min(this._resolution.height / 2, c1pos.y);
          var control1 = this.createPoint((_crd && Ident === void 0 ? (_reportPossibleCrUseOfIdent({
            error: Error()
        ...
        deletePoint() {
          if (this._pointCurveDict.has(this._deleteTarget)) {
            var location = this.getPointLocation(this._deleteTarget);
        ...
        saveBezierPath() {
          ...
          for (var i = 0, len = this._bezierCurveLists.length; i < len; i++) {
            var bezier = this._bezierCurveLists[i]; // 创建一个贝塞尔曲线
            // let bezierCurve = new Bezier(bezier.start, bezier.control, bezier.end, 100);
            // console.log("consscscds", Object.values(bezier));

            var bezierCurve = new (_crd && Bezier === void 0 ? (_reportPossibleCrUseOfBezier({
              error: Error()
            }), Bezier) : Bezier)();
            bezierCurve.init(Object.values(bezier), 2); // 获取曲线点

            var points = bezierCurve.getPoints(this._pointCount);
            console.log("consscscds", this._pointCount); // 获取曲线长度

            var curveLength = bezierCurve.getCurveLength(); // 计算路程长度
        ...
        createPoint(ident, pos, isSelect) {
          if (isSelect === void 0) {
            isSelect = true;
          }
        ...
        getRandPos() {
          var randX = Math.random() * this._resolution.width - this._resolution.width * 0.5;
          var randY = Math.random() * this._resolution.height - this._resolution.height * 0.5;
          return new Vec3(randX, randY, 0);
        } // 初始化一个随机曲线


        initRandCurve() {
          var start = this.createPoint((_crd && Ident === void 0 ? (_reportPossibleCrUseOfIdent({
            error: Error()
        ...
        getPointLocation(node) {
          var curveObj = this._pointCurveDict.get(node);
        ...
        deleteCenterPoint(point) {
          ...
          if (this._pointCurveDict.has(point)) {
            ...
            var CurveObj = this._pointCurveDict.get(point);

            var prevCurve = CurveObj.endCurve;
            var nextCurve = CurveObj.startCurve; // 把前一个曲线的终点移动到后一个曲线的终点上

            prevCurve.end = nextCurve.end; // 重新赋值该节点下的曲线对象的end曲线

            var prevEndCurveObj = this._pointCurveDict.get(prevCurve.end);
            ...
            for (var key in nextCurve) {
              if (key == "end") continue;
              var _point = nextCurve[key];
        ...
        deleteStartPoint(point) {
          ...
          if (this._pointCurveDict.has(point)) {
            ...
            var startCurve = CurveObj.startCurve;
            CurveObj.endCurve = null; // 删除曲线及其相关的点

            var endCurveObj = this._pointCurveDict.get(startCurve.end);

            endCurveObj.endCurve = null;

            for (var key in startCurve) {
              if (key == "end") continue;
              var _point = startCurve[key];
        ...
        deleteEndPoint(point) {
          ...
          if (this._pointCurveDict.has(point)) {
            ...
            var endCurve = CurveObj.endCurve;
            CurveObj.startCurve = null; // 删除曲线及其相关的点

            var startCurveObj = this._pointCurveDict.get(endCurve.start);

            startCurveObj.startCurve = null;

            for (var key in endCurve) {
              if (key == "start") continue;
              var _point = endCurve[key];
        ...
        deleteCurveFromBezierLists(curve) {
          for (var i = 0, len = this._bezierCurveLists.length; i < len; i++) {
            var _curve = this._bezierCurveLists[i];
...
//# sourceMappingURL=82a2420989e61efa4e3d6a954bdb86519724b7cd.js.map