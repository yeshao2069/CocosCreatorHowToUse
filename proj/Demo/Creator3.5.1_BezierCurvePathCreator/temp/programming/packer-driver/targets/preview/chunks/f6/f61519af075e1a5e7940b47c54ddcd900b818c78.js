    ...
    execute: function () {
      ...
      _export("Bezier", Bezier = class Bezier {
        ...
        resetData() {
          ...
          this._totalLength = 0;
          this._currentRunTime = 0;
          var pos = this._pointArr[0].position;
          this._previousPos = {
            x: pos.x,
            y: pos.y,
            length: 0
        ...
        ComputeBezier(dt, runTime) {
          ...
          var t = this._currentRunTime / runTime;
          var x = 0,
              y = 0; //控制点数组
          ...
          var length = Math.sqrt(Math.pow(this._previousPos.x - x, 2) + Math.pow(this._previousPos.y - y, 2));
          var v2 = {
            x: x,
            y: y,
            length: length
        ...
        getPoints(count) {
          ...
          var dt = this._runTime / count; // 开始分割曲线
...
//# sourceMappingURL=f61519af075e1a5e7940b47c54ddcd900b818c78.js.map