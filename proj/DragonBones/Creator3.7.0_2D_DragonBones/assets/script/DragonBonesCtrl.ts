...
import { _decorator, Component, Node, dragonBones, macro, SystemEventType, systemEvent, Vec3, director, EventTouch, UITransform } from 'cc';
...
export default class DragonBonesCtrl extends Component {
    ...
    _updatePosition () {
        const camera = director.getScene()!.getChildByName("Canvas/Camera")!;
        const pos = this.node.getPosition();
        if (this._speedX !== 0) {
            pos.x += this._speedX;
            var minX = -camera.getComponent(UITransform)!.width / 2;
            var maxX = camera.getComponent(UITransform)!.width / 2;
            if (pos.x < minX) {
                pos.x = minX;
            } else if (pos.x > maxX) {
                pos.x = maxX;
...
export class DragonBullet {
    ...
    update () {
        ...
        const camera = director.getScene()!.getChildByName("Canvas/Camera")!;
        if (
            worldPos.x < -100 || worldPos.x >= camera.getComponent(UITransform)!.width + 100 ||
            worldPos.y < -100 || worldPos.y >= camera.getComponent(UITransform)!.height + 100
        ) {
            this.doClean();
            return true;
        }
